#version 450
#extension GL_EXT_nonuniform_qualifier : enable

layout(location = 0) in vec4 fragColor;
layout(location = 1) in vec2 fragTexCoord;
layout(location = 2) flat in uint fragTextureIndex;

layout(location = 0) out vec4 outColor;

// Bindless texture array (same as the main renderer)
layout(set = 0, binding = 0) uniform sampler2D textures[];

void main() {
    // Sample from the font atlas texture
    vec4 texColor = texture(textures[nonuniformEXT(fragTextureIndex)], fragTexCoord);

    // Use the alpha channel for transparency (white text on transparent background)
    outColor = vec4(fragColor.rgb, fragColor.a * texColor.a);
}